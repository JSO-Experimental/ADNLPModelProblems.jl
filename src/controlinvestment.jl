...
function controlinvestment_autodiff(args...; n::Int=200, type::Val{T}=Val(Float64), adbackend=ADNLPModels.ForwardDiffAD(), kwargs...) where T
  N = div(n, 2)
  h = 1/N
  x0 = 1.0
  gamma = 3
  ...
  return ADNLPModel(f, xi, lvar, uvar, c, zeros(T, N-1), zeros(T, N-1), adbackend=adbackend, name="controlinvestment_autodiff")
end

function controlinvestment_radnlp(args...; n::Int=200, type::Val{T}=Val(Float64), kwargs...) where T
  N = div(n, 2)
  h = 1/N
  x0 = 1.0
  gamma = 3
  ...
  uvar = vcat(x0, Inf*ones(T, N-1), ones(T, N))
  xi = vcat(ones(T, N), zeros(T, N))
  return RADNLPModel(f, xi, lvar, uvar, c, zeros(T, N-1), zeros(T, N-1), name="controlinvestment_radnlp")
end

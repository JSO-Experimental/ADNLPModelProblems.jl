...
function sparsine_autodiff(; n::Int=100, type::Val{T}=Val(Float64), adbackend=ADNLPModels.ForwardDiffAD()) where T
  n ≥ 10 || error("sparsine : n ≥ 10")
  function f(x)
    n = length(x)
    return 0.5 * sum(
  ...
  x0 = ones(T, n) / 2
  return ADNLPModel(f, x0, adbackend=adbackend, name="sparsine_autodiff")
end

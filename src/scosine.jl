...
function scosine_autodiff(; n::Int=100, type::Val{T}=Val(Float64), adbackend=ADNLPModels.ForwardDiffAD()) where T
  n ≥ 2 || error("scosine : n ≥ 2")
  p = zeros(n)
  for i=1:n
    p[i] = exp(6.0 * (i-1) / (n-1))
  ...
  x0 = T.([1/p[i] for i=1:n])
  return ADNLPModel(f, x0, adbackend=adbackend, name="scosine_autodiff")
end

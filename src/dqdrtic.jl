...
function dqdrtic_autodiff(; n::Int=100, type::Val{T}=Val(Float64), adbackend=ADNLPModels.ForwardDiffAD()) where T
  function f(x)
    n = length(x)
    return sum(x[i]^2 + 100 * (x[i+1]^2 + x[i+2]^2) for i=1:n-2)
  end
  x0 = 3 * ones(T, n)
  return ADNLPModel(f, x0, adbackend=adbackend, name="dqdrtic_autodiff")
end

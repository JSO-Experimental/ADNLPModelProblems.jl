...
function ncb20_autodiff(; n::Int=100, type::Val{T}=Val(Float64), adbackend=ADNLPModels.ForwardDiffAD()) where T
  n ≥ 31 || error("ncb20 : n ≥ 31")
  function f(x)
    n = length(x)
    h = 1.0/(n-1)
  ...
  return ADNLPModel(f, x0, adbackend=adbackend, name="ncb20_autodiff")
end

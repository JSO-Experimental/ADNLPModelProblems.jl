...
function clnlbeam_autodiff(args...; n::Int=300, type::Val{T}=Val(Float64), adbackend=ADNLPModels.ForwardDiffAD(), kwargs...) where T
  N = div(n-3, 3)
  h = 1/N
  alpha = 350
  function f(y)
  ...
  xi = zeros(T, 3*N+3)
  return ADNLPModel(f, xi, lvar, uvar, c, zeros(T, 2*N), zeros(T, 2*N), adbackend=adbackend, name="clnlbeam_autodiff")
end

function clnlbeam_radnlp(args...; n::Int=300, type::Val{T}=Val(Float64), kwargs...) where T
    N = div(n-3, 3)
    h = 1/N
    alpha = 350
    function f(y)
    ...
    uvar = vcat(ones(T, N+1), 0.05*ones(T, N+1), Inf * ones(T, N+1))
    xi = zeros(T, 3*N+3)
    return RADNLPModel(f, xi, lvar, uvar, c, zeros(T, 2*N), zeros(T, 2*N), name="clnlbeam_radnlp")
end

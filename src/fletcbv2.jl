...
function fletcbv2_autodiff(; n::Int=100, type::Val{T}=Val(Float64), adbackend=ADNLPModels.ForwardDiffAD()) where T
  n ≥ 2 || error("fletcbv2 : n ≥ 2")
  function f(x)
    n = length(x)
    h = 1.0 / (n + 1)
  ...
  x0 = T.([(i/(n+1.0)) for i=1:n])
  return ADNLPModel(f, x0, adbackend=adbackend, name="fletcbv2_autodiff")
end

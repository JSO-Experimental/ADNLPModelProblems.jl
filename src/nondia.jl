...
function nondia_autodiff(; n::Int=100, type::Val{T}=Val(Float64), adbackend=ADNLPModels.ForwardDiffAD()) where T
  n ≥ 2 || error("nondia : n ≥ 2")
  function f(x)
    n = length(x)
    return (x[1] - 1.0)^2 + sum((100.0*x[1] - x[i-1]^2)^2 for i=2:n)
  ...
  x0 = -ones(T, n)
  return ADNLPModel(f, x0, adbackend=adbackend, name="nondia_autodiff")
end

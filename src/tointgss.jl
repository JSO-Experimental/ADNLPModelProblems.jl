function tointgss_radnlp(; n::Int = 100, type::Val{T} = Val(Float64), kwargs...) where {T}
    ...
    function f(x)
        ...
        return sum(
            (10.0 / (n + 2) + x[i+2]^2) *
            (2.0 - exp(-(x[i] - x[i+1])^2 / (0.1 + x[i+2]^2))) for i = 1:n-2
        )
    end
    x0 = ones(T, n)
...
tointgss_meta = Dict(    :nvar => 100,    :variable_size => false,    :ncon => 0,    :variable_con_size => false,    :nnzo => 100,    :nnzh => 5050,    :nnzj => 0,    :minimize => true,    :name => "tointgss",    :optimal_value => NaN,    :has_multiple_solution => missing,    :is_infeasible => false,    :objtype => :other,      :contype => :unconstrained,    :origin => :unknown,    :deriv => typemax(UInt8),   

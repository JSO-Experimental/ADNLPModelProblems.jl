...
function tquartic_autodiff(; n::Int=100, type::Val{T}=Val(Float64), adbackend=ADNLPModels.ForwardDiffAD()) where T
  n ≥ 2 || error("tquartic : n ≥ 2")
  function f(x)
    n = length(x)
    return (x[1] - 1.0)^2 + sum((x[1]^2 - x[i+1]^2)^2 for i=1:n-2)
  ...
  x0 = ones(T, n)
  return ADNLPModel(f, x0, adbackend=adbackend, name="tquartic_autodiff")
end

...
function liarwhd_autodiff(; n::Int=100, type::Val{T}=Val(Float64), adbackend=ADNLPModels.ForwardDiffAD()) where T
  n ≥ 2 || error("liarwhd : n ≥ 2")
  function f(x)
    n = length(x)
    return sum(4.0*(x[i]^2 - x[1])^2 + (x[i] - 1)^2  for i=1:n)
  ...
  x0 = ones(T, n)
  return ADNLPModel(f, x0, adbackend=adbackend, name="liarwhd_autodiff")
end

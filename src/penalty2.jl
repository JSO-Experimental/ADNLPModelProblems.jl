...
function penalty2_autodiff(; n::Int=100, type::Val{T}=Val(Float64), adbackend=ADNLPModels.ForwardDiffAD()) where T
  n ≥ 3 || error("penalty2 : n ≥ 3")
  function f(x)
    n = length(x)
    a = 1.0e-5
  ...
  x0 = ones(T, n) / 2
  return ADNLPModel(f, x0, adbackend=adbackend, name="penalty2_autodiff")
end

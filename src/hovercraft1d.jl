...
function hovercraft1d_autodiff(args...; n::Int=200, type::Val{T}=Val(Float64), adbackend=ADNLPModels.ForwardDiffAD(), kwargs...) where T
    N = div(n, 3)
  function f(y)
  ...
  xi = zeros(T, 3*N-1)
  return ADNLPModel(f, xi, c, zeros(2*N+2), zeros(2*N+2), adbackend=adbackend, name="hovercraft1d_autodiff")
end

function hovercraft1d_radnlp(args...; n::Int=200, type::Val{T}=Val(Float64), kwargs...) where T
    N = div(n, 3)
  function f(y)
    x, v, u = y[1:N], y[N+1:2*N], y[2*N+1:end]
    return sum(u.^2)
  ...
  xi = zeros(T, 3*N-1)
  return RADNLPModel(f, xi, c, zeros(2*N+2), zeros(2*N+2), name="hovercraft1d_radnlp")
end
